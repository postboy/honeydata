...
extern int main(void)
{
	...
	//let orig_array contain numbers from -20000000000000 to 10000000000000 distributed uniformly
	for (j = 0; j < size; j++) {
		/*write a fresh random element to this position until it will be between -20000000000000
		and 10000000000000*/
		while ( (orig_array[j] < -20000000000000) || (orig_array[j] > 10000000000000) )
			randombytes((unsigned char *)(orig_array+j), sizeof(ITYPE));
		}
	
	encode_int64_uniform(orig_array, encoded_array, size, -20000000000000, 10000000000000);
	//get a statistics on an encoded array
	stats_uint8_array((uint8_t *)encoded_array, 2*BYTESIZE, out_stats);
	decode_int64_uniform(encoded_array, decoded_array, size, -20000000000000, 10000000000000);

...
extern int main(void)
{
	...
	const ITYPE orig_array[] = {-110, -110, -110, -112, -112, -110, -110, -110, -112, -112};
	...
	if ((rv = encode_int8_arbitrary(orig_array, &encoded_array, size, min, max, weights)) != 2) {
		error("unexpected output type");
		printf("%d\n", rv);
		test_error();
		}
	decode_int8_arbitrary(encoded_array, decoded_array, size, min, max, weights);
	...
	if ((rv = encode_int8_arbitrary(orig_array, &encoded_array, size, min, max, weights)) != 4) {
		error("unexpected output type");
		printf("%d\n", rv);
		test_error();
		}
	...
	if ((rv = encode_int8_arbitrary(orig_array, &encoded_array, size, min, max, weights)) != 8) {
		error("unexpected output type");
		printf("%d\n", rv);
		test_error();
		}
	...
	if ((rv = encode_int8_arbitrary(orig_array, &encoded_array, size, min, max, weights)) != -1) {
		error("unexpected output type");
		printf("%d\n", rv);
		test_error();
		}

...
extern int main(void)
{
	...
	//try to open file 'uint8_encoding.ods' for writing
	...
	//try to open file 'uint16_encoding.ods' for writing
	if ((fp = fopen("uint16_encoding.ods", "w")) == NULL) {
		fprintf(stderr, "test: fopen error: can't open file 'uint16_encoding.ods' for writing.\n");
	    return 1;
		}
		
	//compare actual vs. ideal distributions of output array
	if (fprintf(fp, "=CHITEST(A2:A65537;B2:B65537)\n") < 0) {
		fprintf(stderr, "test: fwrite error: cannot write to 'uint16_encoding.ods' file.\n");
		if (fclose(fp) == EOF)
			perror("test: fclose error");
		return 1;
		}
